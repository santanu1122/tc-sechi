...
#import "SEPaymentsViewController.h"
#import "SEPaymentViewController.h"
#import "SEPaymentTableViewCell.h"
#import "SEPayment.h"
#import "UIView+Hierarchy.h"

@interface SEPaymentsViewController ()
 ...
 */
@property (strong, nonatomic) SEPaymentTableViewCell* temporaryCell;
...
@implementation SEPaymentsViewController
...
- (void) addButtonTouchedUpInside: (id) sender {
    [self performSegueWithIdentifier:SEPushPaymentAddViewControllerSegue
                              sender:self];
}
...
- (void) alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if(buttonIndex != alertView.cancelButtonIndex) {
        SEPayment* payment = [self.fetchedResultsController objectAtIndexPath:self.indexPathToRemove];
...
- (UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SEPaymentTableViewCell* cell = [tableView dequeueReusableCellWithIdentifier:SEPaymentTableViewCellIdentifier];
    cell.delegate = self;
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"SEPayment" inManagedObjectContext:self.managedObjectContext];
...
{
    if([cell isKindOfClass:[SEPaymentTableViewCell class]]) {
        SEPayment *payment = [self.fetchedResultsController objectAtIndexPath:indexPath];
        SEPaymentTableViewCell* paymentCell = (SEPaymentTableViewCell*) cell;
        ...
        NSString* statusImageName = [payment.statusC isEqualToString:@"Complete"] ? @"icon_status_small_active" : @"icon_status_small";
        paymentCell.paymentStatusImage.image = [UIImage imageNamed:statusImageName];
        ...
        paymentCell.bottomCellView.backgroundColor = [UIColor colorWithRed:0.85 green:0.109 blue:0.36 alpha:1];
    }
}
...
{
    if([[segue destinationViewController] isKindOfClass:[SEPaymentViewController class]] && [sender isKindOfClass:[UITableViewCell class]]) {
        NSIndexPath* indexPath = [self.tableView indexPathForCell:sender];
        SEPayment* payment = [self.fetchedResultsController objectAtIndexPath:indexPath];
        SEPaymentViewController* vc = (SEPaymentViewController*)[segue destinationViewController];
        vc.payment = payment;
    }
}
