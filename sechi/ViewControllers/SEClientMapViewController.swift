...
class SEClientMapViewController: SEViewController, MKMapViewDelegate, UITextViewDelegate {
	 ...
	 */
	var clientAnnotationView = MKPinAnnotationView(annotation: self.clientAnnotation, reuseIdentifier: "nil")
	...
	func updateMapCompletion(completion: () -> ()) {
	    ...
	    if addressComponents.count > 1 {
	        ...
	        placeDictionary["Street"] = setObject:self.addressTextView.text
	    }
	    
	    var geocoder = CLGeocoder()
	    geocoder.geocodeAddressDictionary(placeDictionary) {
	    	(placemarks: Array<CLPlacemark>, error: NSError) -> () in
