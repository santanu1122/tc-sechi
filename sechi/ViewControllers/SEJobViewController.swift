...
class SEJobViewController: SEViewController, UITableViewDataSource, UITableViewDelegate, UICollectionViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIAlertViewDelegate, SESwipeableTableViewCellDelegate, UIGestureRecognizerDelegate {
	 ...
	 */
	func startCameraControllerFromViewController(controller: UIViewController, withCameraCaptureMode cameraCaptureMode: UIImagePickerControllerCameraCaptureMode, usingDelegate delegate: UIImagePickerControllerDelegate | UINavigationControllerDelegate) -> Bool {
	    // check args
	    if !UIImagePickerController.isSourceTypeAvailable(.Camera) || !delegate == nil || !controller {
	        return false
	    }
	 ...
	 */
	func startMediaBrowserFromViewController(controller: UIViewController, photos showPhotos: Bool, videos showVideos: Bool, usingDelegate delegate: UIImagePickerControllerDelegate | UINavigationControllerDelegate) -> Bool {
	    if !UIImagePickerController.isSourceTypeAvailable(.SavedPhotosAlbum) || !delegate || !controller {
	        return false
	    }
	...
	func uuid() -> String {
	    ...
	    CFStringRef uuidStringRef = CFUUIDCreateString(0, uuidRef)
	    CFRelease(uuidRef)
