...
class SEClientAddViewController: SEViewController, UITableViewDataSource, UITableViewDelegate, UITextViewDelegate {
	...
	func setupCells() {
	    ...
	    self.emailCell = self.tableView.dequeueReusableCellWithIdentifier(SETextFieldTableViewCellIdentifie)
	...
	func saveButtonTouchedUpInside(sender: UIButton) {
	    self.client = NSEntityDescription.insertNewObjectForEntityForName("SEClient", inManagedObjectContext: SERestClient.instance.managedObjectContext)
